...
x000154_g_MenpaiWantHonour = 5000 --ĂÅÅÉÊ±×°Đè̉ªµÄÈÙÓ₫
x000154_g_MenpaiGoodsIndex={10124142,10124143,10124144,10124145,10124146,
														10124147,10124148,10124149,10124150}

x000154_g_WantHonour = 100 --Đè̉ªµÄÈÙÓ₫
x000154_g_GiveBanggong = 50 --¸øµÄ°ï¹±

x000154_g_BuyLuoyangMoney = 10000000 --1000½đ

x000154_g_LuoyangOwnerIndex = 53 --ÂåÑô³Ç¹éÊô°ïÅÉIDÔÚGlobalDataÀïĂæµÄË÷̉ư
x000154_g_BaseGuildIndex = 10000 --°ïÅÉIDµÄ»ù´¡Öµ£¬̣̉ÎªGlobalDataÀïĂæ³ơÊ¼ÖµÊÇ0£¬¶ø0ÊÇºÏ·¨°ïÅÉID£¬̣̉´ËÎ̃·¨ÅĐ¶ÏÊÇ·ñÓĐÈË̉Ñ¾­ẬÁË¡£°ïÅÉID¼ÓÉÏ̉»¸ö»ù´¡Öµ£¬ƠâÑù¿É̉ÔÓĂ0À´ÅĐ¶ÏÊÇ·ñ¹ºẬ

--³ÆºÅ½±Àø
x000154_g_PrizeTitle = {AwardPos = 18, SetPos = 42, Index = 255}--½±ÀøµÄË÷̉ư(½Å±¾º¯ÊưÖĐµÄcaseË÷̉ư)£¬ÉèÖĂÎ»ÖĂµÄË÷̉ư(_TITLEĂ¶¾ÙÖĐµÄÖµ)£¬³ÆºÅË÷̉ư(CharTitle.txt±íÖĐµÄË÷̉ư)

x000154_g_LuoyangCityMoneyJZ = 200000 --Á́È¡µÄÂåÑô³ÇÊĐ¹¤×Ê 20½đ½»×Ó
...
function x000154_UpdateEventList( sceneId, selfId, targetId )
	...
	--AddNumText(sceneId, x000154_g_scriptId, "¨¢¨¬¨¨?3??¡Â3?o?", 6, 14); --¨¢¨¬¨¨?3??¡Â3?o?
	--AddNumText(sceneId, x000154_g_scriptId, "¨¢¨¬¨¨?3?¨ºD1¡è¡Á¨º", 6, 15); --¨¢¨¬¨¨?3?¨ºD1¡è¡Á¨º
...
--			else
--				x000154_NotifyFailBox(sceneId, selfId, targetId, "    ¿Û³ư½đÇ®Ê§°Ü£¡")
--			end
--		end
...
--		end
	...
	for i, findId in x000154_g_eventList do
		if eventId == findId then
			CallScriptFunction( eventId, "OnDefaultEvent",sceneId, selfId, targetId, GetNumText() )	--GetNumText()ÊÇaddnumtextÖĐ×îºóµÄ±äÁ¿
			return
		end
	end
...
function x000154_CheckGetMoneyJZ( sceneId, selfId, targetId )
	...
	--Á́È¡¹¤×ÊµÄÊ±¼ä·¶Î§ ÖÜÈƠµÄ0£º00µ½24£º00
